...
class Instructor:
    ...
    def __init__(self, option):  # prepare for training the model
        train_data, test_data = self.load_data(option.dataset, directory=option.directory, percentage=option.percentage)
        self._initialization(option, train_data, test_data)
        self._print_args()
    ...
    def load_data(dataset,
        ...
        ret = []
        # splits = ['SST2_Train.json', 'SST2_Test.json']
        splits = [
        ...
        for split_file in splits:
            ...
            examples = []
            if full_filename.endswith("Train.json") and percentage > 1:  # oversampling and subsampling
    ...
    def _initialization(self, option, train_data, test_data):
        # sentiment label encoder
        senti_label_corpus = [trans_label[option.dataset][row["polarity"]] for row in train_data]
        senti_label_encoder = LabelEncoder(senti_label_corpus, reserved_labels=[], unknown_index=None)
        option.label_class = len(senti_label_encoder.vocab)

        if option.model_type.lower() == "bert":
            model = AttBERTForPolarity(option).to(option.device)
            tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        elif option.model_type.lower() == "roberta":
            model = ROBERTAForPolarity(option).to(option.device)
        ...
        label_token2idx = senti_label_encoder.stoi
        list_labels = list(label_token2idx.keys())  # list_labels: ['negative', 'positive']
        ...
        for idx, a_data in enumerate(chain(train_data, test_data)):
            ...
            # a_data: {'polarity': '0', 'sentence': 'hide new secretions from the parental units', 'tokens_length': 13,
            # 'tokens_list': ['[CLS]', 'negative', 'positive', '[SEP]', 'hide', 'new', 'secret', '##ions', 'from',
            # 'the', 'parental', 'units', '[SEP]']}
        ...
        for idx, a_data in enumerate(chain(train_data, test_data)):
            ...
            if idx < N_train:
                ...
                if option.class_use_bert_embedding == 1:
                    idx_label = list_labels.index(trans_label[option.dataset][label])
        ...
        drop_last = True if len(trainset) > option.batch_size else False
        train_dataloader = DataLoaderX(dataset=trainset, batch_size=option.batch_size,
                                       shuffle=True, num_workers=4, pin_memory=True, drop_last=drop_last,
                                       collate_fn=lambda x: collate_fn(x, label_length, label_start,
                                                                       label_end,
                                                                       option.label_class))  # training dataloader
            ...
            if param.requires_grad and self.opt.model_type not in name:
                if 'embedding' in name:  # treat embedding matrices as special cases
                    weight = torch.nn.init.xavier_uniform_(
                        torch.zeros_like(param))  # use xavier_uniform to initialize embedding matrices
                    weight[0] = torch.tensor(0, dtype=param.dtype,
                                             device=param.device)  # the vector corresponding to padding index should be zero
    ...
    def _train(self, optimizer, criterion, scaler, warm_up):
        ...
        for sample_batched in self.train_dataloader:  # mini-batch optimization
            ...
            if warm_up:
                keywords_labels = torch.where(mask_pos == False, -100 * torch.ones_like(mask_pos), keywords_labels)
                del mask_pos
                ...
                with torch.cuda.amp.autocast():
                    ...
                    # word feature
                    word_feature = torch.gather(word_feature, dim=1,
                                                index=inputs_id_order.unsqueeze(-1).expand(-1, -1, HS))
                    # label feature
                ...
                if self.opt.alpha1 > 1e-32:
                    n_mlm_loss += mlm_loss.item() * len(labels)
                if self.opt.alpha2 > 1e-32 and len(torch.unique(labels)) > 1:
                    list_n_con_loss = [con_loss.item() * len(labels) + prev_con_loss for con_loss, prev_con_loss in
                                       zip(list_con_loss, list_n_con_loss)]
                n_train_loss += train_loss.item() * len(labels)
                n_correct += (torch.argmax(predicts, -1) == labels).sum().item()  # update correct sample number
                n_train += len(labels)  # update train sample number
        return n_train_loss / n_train, n_ce_loss / n_train, n_mlm_loss / n_train, [n_con_loss / n_train for n_con_loss
                                                                                   in
                                                                                   list_n_con_loss], n_correct / n_train, time.time() - t
    ...
    def _contrast_loss(self, cls_feature, label_feature, labels):
        ...
        normed_positive_label_feature = torch.gather(normed_label_feature, dim=1,
                                                     index=labels.reshape(-1, 1, 1).expand(-1, 1, HS)).squeeze(
            1)  # (bs, 768)
    ...
    def _predict(self, eval_set):
        self.model = torch.load('results_polarity/model_8.pkl', map_location='cuda:0')
        ...
        self.model.eval()
        with torch.no_grad():
            for sample_batched in eval_set:  # mini-batch optimization
                if self.opt.device == "cuda":
                    inputs = list(map(lambda x: x.cuda(non_blocking=True), sample_batched))
                else:
                    inputs = list(sample_batched)
                ...
                # predicts = torch.bmm(label_feature, cls_feature.unsqueeze(-1)).squeeze(-1)
                predicts = torch.bmm(label_feature, self.model.fc_dropout(cls_feature.unsqueeze(-1))).squeeze(-1)
    ...
    def run(self):
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            train_loss, ce_loss, mlm_loss, list_con_loss, train_acc, used_time = self._train(optimizer, criterion,
                                                                                             scaler, warm_up)
            ...
            if test_acc >= best_test_acc:
                ...
                if best_test_acc > all_best_acc:
                    ...
                    torch.save(self.model, f'results_polarity/{self.opt.dataset}/model_{epoch}.pkl')
            else:
                patience += 1
                if patience > 50:
