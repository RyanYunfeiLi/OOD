...
class Evaluation(object):
    ...
    def __init__(self, option):  # prepare for training the model
        dev_data = self.load_data(dataset=option.dataset, directory=option.directory)
        self._initialization(option, dev_data)
        self._print_args()
    ...
    def load_data(dataset,
        ...
        ret = []
        # splits = ['SST2_Dev.json']
        splits = [
            '_'.join([dataset, fn_]) for (requested, fn_) in [(dev, dev_file), ]
            if requested
        ]
    ...
    def _initialization(self, option, dev_data):
        # don't need sentiment label encoder
        senti_label_corpus = [trans_label[option.dataset][row["polarity"]] for row in dev_data]
        ...
        model.eval()
        max_length = 0
        label_token2idx = senti_label_encoder.stoi  # label_token2idx: {'negative': 0, 'positive': 1}
        ...
        N_test = len(dev_data)
        for idx, a_data in enumerate(chain(dev_data)):
