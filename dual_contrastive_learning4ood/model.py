...
class AttBERTForPolarity(nn.Module):
    def __init__(self, opt):
        ...
        bert = BertModel.from_pretrained('/diskb/liyunfei/jupyter/Experiment/GOT-main/pretrained/bert-base-uncased')
        for param in bert.parameters():
            param.requires_grad = True
        ''' fc_out '''
        self.fc_dropout = nn.Dropout(opt.fc_dropout)
        ''' bert lm model '''
        self.bert_lm_model = BertForMaskedLM.from_pretrained('/diskb/liyunfei/jupyter/Experiment/GOT-main/pretrained'
                                                             '/bert-base-uncased').to(opt.device)
