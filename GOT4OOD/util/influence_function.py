...
class InF():
    ...
    def init_loader(self):
        ...
        self.loaders['merge'] = torch.utils.data.DataLoader(dataset=merge_dataset, batch_size=1, shuffle=False,
                                                            drop_last=False)
        ...
        if len(os.listdir(self.s_dir)) == len(self.loaders['valid'].dataset.y):
            ...
            for index, (_, y) in enumerate(self.loaders['valid']):
                if index >= self.start_index and index < self.end_index:
                    print(
                        '{}: {}/{}'.format(self.args.ith, index - self.start_index, self.end_index - self.start_index))
    ...
    def cal_s_single(self, x_test, y_test):
        ...
        for i in range(self.args.repeat):
            ...
            for j, (_, y) in enumerate(self.loaders['merge']):
                ...
                h_estimate = [_v + (1 - self.args.damp) * _h_e - _hv / self.args.scale for _v, _h_e, _hv in
                              zip(v, h_estimate, hv)]
    ...
    def cal_influence(self):
        ...
        normalized_influences = [1 / (1 + math.exp(self.args.gamma * influence / (max_influence - min_influence))) for
