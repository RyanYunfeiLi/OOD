            ...
            if split in rep_list and not self.args.regenerate:
                ...
                if split == 'ood':
                    dataset = ClassifierDataSet(
                        'output/generating/{}/ood_{}.csv'.format(self.args.dataset, self.args.seed))
                else:
                    dataset = ClassifierDataSet(self.args.train.replace('train', split))
                loader = torch.utils.data.DataLoader(dataset=dataset, batch_size=self.args.batch_size, shuffle=False,
                                                     drop_last=False)
                for x, _ in loader:
