...
class Train():
    ...
    def __init__(self, config):
        ...
        if self.args.pretrained != '':
            model_param = torch.load(self.args.pretrained,
    ...
    def save_model(self, result):
        ...
        if is_exists:
            ...
            dir_name = 'output/params/{}/{}_{}'.format(self.args.dataset, result,
    ...
    def train_with_ood(self):
        """ ind: in dataset, ood: out of dataset """
        main_losses = []
        auxiliary_losses = []
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.epoch + 1):
            ...
            print("[Epoch {}] Train Main Loss={} Auxiliary Loss={} T={}s".format(epoch, np.mean(main_losses),
                                                                                 np.mean(auxiliary_losses), t1 - t0))
            self.mdl.eval()
    ...
    def test_wo_save_model(self):
        self.mdl.eval()
        model_param = torch.load(self.args.pretrained, map_location='cuda:{}'.format(self.args.gpu) \
            if self.args.gpu != -1 else 'cpu')
        self.mdl.load_state_dict(model_param)
        aupr_out = self.evaluate(self.test_loader)
        # aupr_out = self.evaluate(self.valid_loader)
        return aupr_out
...
if __name__ == '__main__':
    ...
    exp.train()
    exp.test()
    # aupr_out = exp.test_wo_save_model()
    # print(aupr_out)
